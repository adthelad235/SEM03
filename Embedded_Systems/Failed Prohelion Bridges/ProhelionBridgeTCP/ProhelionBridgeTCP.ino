...
// UDP single-frame packet: 7 + 7 + 14 = 28 bytes
#define UDP_PKT_LEN     28

// TCP initial header: 4 (fwd_id) + 4 (fwd_range) + 7 (bus_id) + 7 (client_id) = 22 bytes
...
static void parseTcpHeader() {
    // 4 bytes fwd_id (BE) + 4 bytes fwd_range (BE) + 7 bus_id + 7 client_id
    ...
    tcpFwdRange = ((uint32_t)tcpRxBuf[4] << 24) |
                  ...
                  ((uint32_t)tcpRxBuf[6] <<  8) |
                   (uint32_t)tcpRxBuf[7];
    // bus_id at [8..14], client_id at [15..21] - we don't need to validate them
    // strictly; Profinity sets them to identify itself.
